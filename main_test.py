...
def main() -> None:
    ...
    print("\nTo solve the QP, install qiskit and a solver backend and call the optimizer of your choice.")
...
def normalize_cpu_demands(tasks: List[Dict[str, float]], m: int, cpu_max: float) -> None:
    ...
    # Iteratively scale non-clipped tasks
    safety = 0.999
    # Identify tasks not at cap
    while True:
...
def build_qubo_surrogate(tasks: List[Dict[str, float]], m: int, expansion: Dict[int, Tuple[float, float, float]] = None,
    ...
    # assignment completeness penalties (sum_s x_i_s == 1)
    for i in range(n):
        for j in range(m):
            vi = f"x_{i}_{j}"
            # linear part from penalty
...
def can_fit_on_server_state(st: Dict[str, Any], job: Dict[str, Any]) -> bool:
    ...
    Args:
        st: Server state dict with keys: cores, ram, nvme, bandwidth.
        job: Job dict with keys: cores, ram, nvme, bandwidth.

    Returns:
        True if job fits, False otherwise.
    """
    return (
        st['cores'] + job.get('cores', 0) <= CORES_PER_SERVER and
        st['ram'] + job.get('ram', 0.0) <= RAM_PER_SERVER and
        st['nvme'] + job.get('nvme', 0.0) <= NVME_PER_SERVER and
        st['bandwidth'] + job.get('bandwidth', 0.0) <= BANDWIDTH_PER_SERVER
    )
...
def online_scheduler_with_arrivals(jobs: List[Dict], servers: int, verbose: bool = True) -> Tuple[Dict[int, Dict], List[str], Dict[str, float]]:
    ...
    """
    # Use module-level heapq and mean and constants
    ...
    def snapshot_server(sidx: int) -> str:
        """Snapshot a server's resource state at current time."""
        return format_server_snapshot(sidx, server_state[sidx], current_time)
    ...
    def assign_job_to_server(job: Dict[str, Any], sidx: int, start_time: float) -> None:
        ...
        # update peak core utilization
        cores_pct = (st['cores'] / CORES_PER_SERVER) * 100.0 if CORES_PER_SERVER > 0 else 0.0
...
# ---------------------------------------------------------------------------
def solve_binpack_qaoa(jobs: List[Dict[str, Any]], num_servers: int) -> Dict[str, Any]:
    ...
    try:
        ...
        return _classical_fallback(jobs, num_servers, reason=str(e))

    J = len(jobs)
    S = num_servers

    # Cap problem size — QAOA scales as O(2^n) on simulator
    # For large problems fall back to classical to keep response times reasonable
    if J * S > 20:
        return _classical_fallback(
            jobs, num_servers,
            reason=f"Problem too large for simulator ({J} jobs x {S} servers = {J*S} vars). Using classical greedy."
        )

    qp = QuadraticProgram(name="bin_packing")

    # Binary variables: x_j_s (job j on server s), y_s (server s active)
    for j in range(J):
        for s in range(S):
            qp.binary_var(name=f"x_{j}_{s}")
    for s in range(S):
    ...
    # Objective: minimize number of active servers
    obj = {f"y_{s}": 1.0 for s in range(S)}
    qp.minimize(linear=obj)

    # Constraint 1: each job assigned to exactly one server
    for j in range(J):
        lhs = {f"x_{j}_{s}": 1.0 for s in range(S)}
        qp.linear_constraint(linear=lhs, sense="==", rhs=1.0, name=f"assign_{j}")

    # Constraint 2: server resource limits (cores, ram, nvme)
    for s in range(S):
        cores_lhs = {f"x_{j}_{s}": float(jobs[j].get("cores", 0)) for j in range(J)}
        cores_lhs[f"y_{s}"] = -float(CORES_PER_SERVER)
        ...
        ram_lhs[f"y_{s}"] = -float(RAM_PER_SERVER)
    ...
    try:
        ...
        return _classical_fallback(jobs, num_servers, reason=f"QAOA solve failed: {e}")

    # Parse result into assignments
    assignments = {}
    timeline = []
    x_vals = {var.name: int(round(val)) for var, val in zip(qp.variables, result.x)}
    ...
    timeline.append(f"Objective value: {result.fval:.4f}")

    # Compute stats
    makespan = max((job.get("duration", 0.0) for job in jobs), default=0.0)
    peak_util = min((sum(
        jobs[j].get("cores", 0) for j in range(J) if assignments.get(jobs[j]["id"]) == s
    ) / CORES_PER_SERVER * 100 for s in range(S)), default=0.0)
...
def _classical_fallback(jobs: List[Dict[str, Any]], num_servers: int, reason: str = "") -> Dict[str, Any]:
...
def schedule(req: WorkloadRequest):
    ...
    _self.CORES_PER_SERVER     = req.server_constants.cores
    _self.RAM_PER_SERVER       = req.server_constants.ram
    _self.NVME_PER_SERVER      = req.server_constants.nvme
    _self.BANDWIDTH_PER_SERVER = req.server_constants.bandwidth
...
def schedule_quantum(req: WorkloadRequest):
    ...
    jobs = generate_jobs(req.profile_counts)
    result = solve_binpack_qaoa(jobs, num_servers=req.servers)
    return result
...
def schedule_compare(req: WorkloadRequest):
    ...
    quantum_result = solve_binpack_qaoa(jobs, num_servers=req.servers)
