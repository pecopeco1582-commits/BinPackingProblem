    ...
    for j, job in enumerate(jobs):
        ...
        for s in range(S):
            if x_vals.get(f"x_{j}_{s}", 0) == 1:
                ...
                timeline.append(f"{solver_label}: Job {job['id']} ({job['type']}) -> Server {s}")
                assigned = True
                break
        if not assigned:
            # Solver returned no valid assignment for this job.
            # Pick the server with the most remaining capacity (greedy repair).
            server_load = {s: sum(1 for jid, srv in assignments.items() if srv == s)
                           for s in range(S)}
            best_s = min(server_load, key=server_load.get)
            assignments[job["id"]] = best_s
            timeline.append(f"{solver_label}: Job {job['id']} ({job['type']}) -> Server {best_s} (repaired)")
